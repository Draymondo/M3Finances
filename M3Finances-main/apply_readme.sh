...
# Adds README.md (full changelog vs upstream) and removes the upstream LICENSE file
# (personal, unpublished fork).
# Run this from the ROOT of your git clone (where settings.gradle.kts lives).
set -e
echo "Removing LICENSE (personal fork, not for redistribution)..."
rm -f LICENSE

echo "Writing README.md..."
cat > "README.md" << 'CLAUDE_README_APPLY_EOF'
# M3Finances
...
CLAUDE_README_APPLY_EOF

echo "Done. Now run: git add -A && git commit -m \"Add README, remove upstream LICENSE\" && git push"
