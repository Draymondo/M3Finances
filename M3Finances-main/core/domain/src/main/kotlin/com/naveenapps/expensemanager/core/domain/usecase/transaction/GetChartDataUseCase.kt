...
class GetChartDataUseCase(
    ...
    fun invoke(): Flow<AnalysisData> {
        return combine(
            ...
            val accountingTransactions = transactions.expandedForCategoryAccounting()
            val bucketTotals = HashMap<String, DoubleArray>(minOf(accountingTransactions.size, MAX_CHART_POINTS))
            val transactionItems = ArrayList<TransactionUiItem>(accountingTransactions.size)

            accountingTransactions.forEach { item ->
            ...
            val earliestTransactionMs = accountingTransactions.minOf { it.createdOn.time }
            val latestTransactionMs = accountingTransactions.maxOf { it.createdOn.time }
