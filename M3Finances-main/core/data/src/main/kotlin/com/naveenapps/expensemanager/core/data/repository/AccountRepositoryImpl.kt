...
class AccountRepositoryImpl(
    ...
    override suspend fun deleteAccount(account: Account): Resource<Boolean> =
        withContext(dispatchers.io) {
            return@withContext try {
                // Les transactions liées à ce compte vont être supprimées en cascade (FK CASCADE).
                // On compense d'abord leur impact sur les soldes des comptes concernés,
                // sinon les autres comptes (ex: transferts) gardent un solde incohérent.
                val relatedTransactions = transactionDao.getTransactionsByAccountId(account.id)
                relatedTransactions.forEach { transactionEntity ->
