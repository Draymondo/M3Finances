...
class CategoryRepositoryImpl(
    ...
    override suspend fun deleteCategory(category: Category): Resource<Boolean> =
        withContext(dispatchers.io) {
            return@withContext try {
                // Les transactions liées à cette catégorie vont être supprimées en cascade (FK CASCADE).
                // On compense d'abord leur impact sur les soldes des comptes concernés.
                val relatedTransactions = transactionDao.getTransactionsByCategoryId(category.id)
