...
interface BudgetDao : BaseDao<BudgetEntity> {
    ...
    /** Snapshot variant of [getBudgets] for the cloud sync. */
    @Query("SELECT * FROM budget ORDER BY created_on DESC")
    suspend fun getAllBudgetEntities(): List<BudgetEntity>
    ...
    @Query("DELETE FROM budget_category_relation")
    suspend fun deleteAllBudgetCategories()

    @Query("DELETE FROM budget_account_relation")
    suspend fun deleteAllBudgetAccounts()
