...
private fun CategorySelectionView(
    ...
    SafeModalBottomSheet(
        ...
        CategorySelectionScreen(
            ...
            selectedCategory = state.splitCategorySelectionIndex?.let { state.splitItems.getOrNull(it)?.category }
                ?: state.selectedCategory,
            createNewCallback = {
                onAction.invoke(TransactionCreateAction.OpenCategoryCreate(null))
            },
            onItemSelection = {
                state.splitCategorySelectionIndex?.let { index ->
                    onAction.invoke(TransactionCreateAction.SelectSplitCategory(index, it))
...
private fun TransactionCreateContent(
    ...
    Column(
        ...
        SettingsSection(title = stringResource(R.string.details)) {
            AppCardView {
                Column(
                    ...
                    state.aiScanError?.let { errorMsg ->
                        Text(
                            text = "⚠ $errorMsg",
        ...
        AnimatedVisibility(
            visible = state.transactionType != TransactionType.TRANSFER && !state.isSplit,
        ...
        AnimatedVisibility(
            ...
            SettingsSection(title = stringResource(R.string.split_transaction)) {
                Row(
                    modifier = Modifier.fillMaxWidth(),
                    horizontalArrangement = Arrangement.SpaceBetween,
                ) {
                    Text(text = stringResource(R.string.split_transaction))
                    Switch(
                        checked = state.isSplit,
                        onCheckedChange = { onAction(TransactionCreateAction.ToggleSplit) },
                    )
                }
            }
        }
        ...
        AnimatedVisibility(
            visible = state.isSplit && state.transactionType != TransactionType.TRANSFER,
            enter = fadeIn(tween(200)) + expandVertically(tween(250)),
            exit = fadeOut(tween(150)) + shrinkVertically(tween(200)),
        ) {
            SettingsSection(title = stringResource(R.string.split_lines)) {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                            ...
                            Column(modifier = Modifier.padding(12.dp)) {
                                Row(
                                    ...
                                    CategoryItem(
                                        ...
                                        modifier = Modifier.weight(1f),
                                        onClick = {
                                            onAction(TransactionCreateAction.ShowSplitCategorySelection(index))
                                        },
                                        trailingContent = { CategoryItemDefaults.ChevronTrailing() },
                                    )
                                    if (state.splitItems.size > 2) {
                                        IconButton(onClick = { onAction(TransactionCreateAction.RemoveSplitItem(index)) }) {
                                ...
                                DecimalTextField(
                                    ...
                                    isError = item.amount.valueError,
                                    onValueChange = { onAction(TransactionCreateAction.SetSplitAmount(index, it)) },
                                    label = R.string.amount,
                                )
                                OutlinedTextField(
                                    modifier = Modifier.fillMaxWidth(),
                                    value = item.notes.value,
                                    onValueChange = { onAction(TransactionCreateAction.SetSplitNotes(index, it)) },
                                    label = { Text(stringResource(R.string.split_line_note)) },
                                    singleLine = true,
                                )
                            }
                        }
                    ...
                    Row(
                        ...
                        Text(stringResource(R.string.split_remaining))
                        Text(
                            text = state.splitRemaining.amountString ?: state.splitRemaining.amount.toString(),
                            color = if (state.splitTotalError) MaterialTheme.colorScheme.error
                            else MaterialTheme.colorScheme.onSurface,
                        )
                    }
                    if (state.splitTotalError) {
                        Text(
                            text = stringResource(R.string.split_amount_mismatch),
                            color = MaterialTheme.colorScheme.error,
                        )
                    }
                    Row(
                        ...
                        androidx.compose.material3.TextButton(
                            onClick = { onAction(TransactionCreateAction.AddSplitItem) },
                        ) {
                            Icon(Icons.Default.Add, contentDescription = null)
                            Spacer(Modifier.width(8.dp))
                            Text(stringResource(R.string.add_split_line))
                        }
                    }
                }
            }
