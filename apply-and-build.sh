#!/usr/bin/env bash
# Usage : place ce script ET le patch éventuel à la racine du dépôt
...
if ! git apply --check "$PATCH_FILE" >/tmp/apply-and-build.patchcheck 2>&1; then
    echo "Le patch n'est pas compatible avec ce dépôt actuel."
    echo "Cause probable : les fichiers visés n'existent plus ou le contexte diffère de cette branche."
    echo "Détails du patch :"
    cat /tmp/apply-and-build.patchcheck
    echo ""
    echo "Aucune modification n'a été appliquée au dépôt pour éviter de casser le projet."
    exit 1
fi
...
if git diff --quiet --exit-code --cached && git diff --quiet --exit-code; then
    echo "Aucun changement à committer après application du patch."
    exit 0
fi
git add -A
